...
(() => {
  ...
  async function toDataUrl(file, targetW) {
    ...
    try {
      // เก็บภาพละเอียดเสมอ แม้ช่องเล็ก (เช่นโลโก้ 52px) — อย่างน้อย 1600px หรือเท่าไฟล์จริง
      const cap = Math.min(MAX_DIM, Math.max(Math.round(targetW * 3), 1600));
  ...
  const stylesheet =
    ...
    ':host([nochrome]) .ctl{display:none!important}' +
